...
# 增强日志工具 - 使用可选导入
try:
    from services.common.utils.log_utils import (
        LogContext,
        MemoryLogger,
        log_step,
        log_substep,
    ...
    # Fallback when log_utils is not available
    LogContext = None
    MemoryLogger = None
    log_step = lambda *args, **kwargs: None
    log_substep = lambda *args, **kwargs: None
    log_memory_operation = lambda *args, **kwargs: None
    log_llm_call = lambda *args, **kwargs: None
    SEPARATOR_LIGHT = "-" * 60
    _memory_logger = None
